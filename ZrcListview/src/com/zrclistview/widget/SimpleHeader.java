...
public class SimpleHeader implements Headable {
		...
		case STATE_RELEASE:
			...
			for (int i = 0; i < mPice; i++) {
				...
				if (offset < height * 3 / 4) {
					angleParam = offset * 16 / height - 3;// 姣�1%杞�0.16搴�;
				} else {
					angleParam = offset * 300 / height - 217;// 姣�1%杞�3搴�;
				}
		...
		case STATE_FAIL:
			...
			if (time < 30) {
				...
				String text = mMsg != null ? mMsg
						: mState == STATE_SUCCESS ? "鍔犺浇鎴愬姛" : "鍔犺浇澶辫触";
				float y;
				if (offset < height) {
					y = offset - height / 2;
				} else {
	...
	public void toastResultInOtherWay(Context context, int state) {
		if (state == Headable.STATE_SUCCESS) {
			...
			Toast.makeText(context, mMsg != null ? mMsg : "鍔犺浇澶辫触",
